...
class Christofides():
    ...
    def go(self):
        # build a graph
        G = self.build_graph(self.vertices)
        # print("Graph: ", G)
        ...
        path = self.removeDuplicates(eulerian_tour)
        path.append(path[0])

        length = self.cycleLength(path)

        # print("Result path: ", path)
        # print("Christofides Method: ", length)

        fullPath = []
        for i in path:
            fullPath.append(self.vertices[i])

        return length, fullPath
    ...
    def cycleLength(self, cycle):
        ...
        for i in range(len(cycle) - 1):
            length += math.dist(self.vertices[cycle[i]], self.vertices[cycle[i+1]])
        ...
        for this in range(len(data)):
            for another_point in range(len(data)):
                if this != another_point:
                    ...
                    graph[this][another_point] = math.dist(data[this], data[another_point])
...
if __name__ == '__main__':
    graph = [[1380, 939], [2848, 96], [3510, 1671], [457, 334], [3888, 666], [984, 965], [2721, 1482], [1286, 525],
